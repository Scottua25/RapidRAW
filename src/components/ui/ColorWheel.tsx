...
const ColorWheel = ({
  ...
  useEffect(() => {
    ...
    if (isWheelDragging) {
      window.addEventListener('mouseup', handleWheelDragEnd);
      window.addEventListener('touchend', handleWheelDragEnd);
    }

    return () => {
      window.removeEventListener('mouseup', handleWheelDragEnd);
      window.removeEventListener('touchend', handleWheelDragEnd);
    };
  }, [isWheelDragging]);
