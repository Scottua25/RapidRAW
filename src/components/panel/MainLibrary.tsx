...
function Thumbnail({
  ...
  return (
    <div
      className={`aspect-square cursor-pointer group relative transition-all duration-150 bg-surface rounded-md overflow-hidden ${ringClass}`}
      onClick={(e: any) => {
        e.stopPropagation();
        onImageClick(path, e);
      }}
...
function LoupeViewport({
  ...
  const fittedSize = useMemo(() => {
    ...
    return {
      width,
      height: width / fullAspectRatio,
    };
  }, [fullImageSize, previewSourceSize, viewportSize.height, viewportSize.width]);

  const referenceImageSize = useMemo(() => {
    if (fullImageSize && fullImageSize.width > 0 && fullImageSize.height > 0) {
      return fullImageSize;
    }

    if (previewSourceSize && previewSourceSize.width > 0 && previewSourceSize.height > 0) {
  ...
  const fitZoomPercent = useMemo(() => {
    ...
    if (referenceWidth <= 0 || referenceHeight <= 0 || fittedSize.width <= 0 || fittedSize.height <= 0) {
      return 1;
    }
  ...
  const scaledSize = useMemo(
    () => ({
      width: fittedSize.width * displayScale,
      height: fittedSize.height * displayScale,
    }),
